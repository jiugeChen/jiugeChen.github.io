...
(function ($) {
  ...
  SearchService = function (options) {
    ...
    self.parseSelectors = function () {
      for (let key in self.config.selectors) {
...
(function ($) {
  ...
  AlgoliaSearch = function (options) {
    ...
    let self = this;
    let endpoint = "https://" + self.config.appId + "-dsn.algolia.net/1/indexes/" + self.config.indexName;
    ...
    self.buildResultList = function (data) {
      ...
      $.each(data, function (index, row) {
        let url = row.permalink || row.path || "";
        if (!row.permalink && row.path) {
          url = ROOT + url;
        }
        let title = row.title;
        let digest = row._highlightResult.excerptStrip.value || "";
    ...
    self.query = function (queryText, page, callback) {
      $.get(endpoint, {
        ...
        if (status === 'success' && data.hits && data.hits.length > 0) {
          let results = self.buildResultList(data.hits);
...
(function ($) {
  ...
  AzureSearch = function (options) {
    SearchService.apply(this, arguments);
    let self = this;
    let endpoint = "https://" + self.config.serviceName + ".search.windows.net/indexes/" + self.config.indexName + "/docs?api-version=2015-02-28";
    self.nav.current = 1;
    self.addLogo('azure');
      ...
      $.ajax({
        ...
        success: function (data, status) {
          if (status === 'success' && data.value && data.value.length > 0) {
            let results = self.buildResultList(data.value);
            self.dom.modal_results.html(results);
          }
          else {
            self.onQueryError(queryText, status);
...
(function ($) {
  ...
  BaiduSearch = function (options) {
    ...
    self.loadScript = function () {
      ...
      let script = "<script src='http://zhannei.baidu.com/api/customsearch/apiaccept?sid=" + self.config.apiId + "&v=2.0&callback=customSearch.initBaidu' type='text/javascript' charset='utf-8'></script>";
  ...
  GoogleCustomSearch = function (options) {
    ...
    self.buildResultList = function (data) {
      ...
      $.each(data, function (index, row) {
        let url = row.link;
        let title = row.title;
        let digest = (row.htmlSnippet || "").replace('<br>', '');
        html += self.buildResult(url, title, digest);
      });
      return html;
    };
    ...
    self.query = function (queryText, startIndex, callback) {
      $.get(endpoint, {
        ...
        if (status === 'success' && data.items && data.items.length > 0) {
          let results = self.buildResultList(data.items);
          self.dom.modal_results.html(results);
        }
        else {
          self.onQueryError(queryText, status);
